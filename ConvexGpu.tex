...
memory, therefore we need, we need to transfers some data from the main memory
to them. These communication must be taken into account when we try to write
efficient code on a GPU.  Some work \cite{venkatasubramanian2009tuned} have
shown that we can be more efficient be recomputing the same results several
times if it reduce the number of CPU-GPU communications.  
...
amount of communications that HEFT has to manage. However we also reduce
...
This study focus on applications with a fine grain, where the communication
...
$59\%$. Some contention effects have also been highlighted on an heterogeneous
machine with 8 GPUs. 
...
Finally increase the grain of a DAG using the clustering algorithm
before running HEFT to obtain a schedule. This way, observe a
