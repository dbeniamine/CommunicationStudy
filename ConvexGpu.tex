...
\mytodo{Insister sur l'aspect théorique de cette étude: pas de contention,
utilise beaucoup de graphs random, pas d'exec réel}
...
\mytodo{Insister étude d'algo special com dans context com, + contention et
debut de sol}
...
scheduler and several heuristics within the parallel runtime XKAAPI.  All the
list algorithms cited before are efficient for coarse grain applications
however they suffer bad performances when the grain is fine enough, as they do
not consider the whole structure of the DAG. Therefore our analysis focus on
very fine grain applications. 
...
\mytodo{insister sur situation plein de com}
...
on a machine with $8$ GPUs, and up to $22\%$ on a machine with only one GPU.
...
\mytodo{insister ici amélioration sans toucher l'algo, donc gros potentiel sur
cette piste}
